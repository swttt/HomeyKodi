...
function init() {
  ...
  Homey.manager('flow').on('action.send_notification', onFlowActionSendNotification)
}
      ...
      case 'kodi_new_movies':
        ...
        Homey.manager('drivers').getDriver('kodi').getNewestMovies(null, daysSince)
          .then(function(movies) {
            speech.say(__('talkback.found_following_movies', {
      ...
      case 'kodi_new_episodes':
        ...
        Homey.manager('drivers').getDriver('kodi').getNewestEpisodes(null, daysSinceEpisode)
          .then(function(episodes) {
            speech.say(__('talkback.found_following_episodes', {
              'days_since': daysSinceEpisode
            }))
...
function onFlowActionSendNotification(callback, args) {
  ...
  Homey.manager('drivers').getDriver('kodi').sendNotification(args.kodi.id, args.message)
    .then(function() {
      callback(null, true)
    })
    .catch(function(error) {
